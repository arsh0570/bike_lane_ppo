...
if __name__ == "__main__":
    ...
    for restart in range(n_restarts):
        ...
        env.reset()  # random initial assignment
        prev_reward = env.last_reward
        
        # your existing hill climbing loop (1000 steps) goes here
        for _ in range(1000):
            act_avail = env.get_available_actions()
            action = np.random.choice(act_avail)
            rew = env.step(action)
            if rew < prev_reward:
                env.revert_action(action)
            else:
                prev_reward = rew
        
        if prev_reward > best_overall_reward:
            best_overall_reward = prev_reward
            best_overall_assignment = env.current_assignment.copy()

    print(f"Best over {n_restarts} restarts: {best_overall_reward}")
